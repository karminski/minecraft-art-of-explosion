...
function findSafeSpawnPosition() {
    ...
    let highestY = 0;
    for (let y = worldSize - 1; y >= 0; y--) {
        if (world[spawnX][y][spawnZ] !== blockTypes.air) {
            highestY = y + 1; // 站在方块上方
            break;
        }
    }
...
// 碰撞检测
function checkCollision(position, direction, checkClimb = false) {
    ...
    if (x >= 0 && x < worldSize && y >= 0 && y < worldSize && z >= 0 && z < worldSize) {
        ...
        if (world[x][y][z] !== blockTypes.air) {
            // 检查障碍物上方是否为空气
            if (y + 1 < worldSize && world[x][y + 1][z] === blockTypes.air) {
                // 检查障碍物上方再上一格也是空气(确保有足够头部空间)
                if (y + 2 < worldSize && world[x][y + 2][z] === blockTypes.air) {
                    // 检查障碍物上方是否有空间可站立
            ...
            // 检查是否超过2格高度
            if (y + 1 < worldSize && world[x][y + 1][z] !== blockTypes.air &&
                y + 2 < worldSize && world[x][y + 2][z] !== blockTypes.air) {
...
function updateCamera() {
    ...
    // 检测前方碰撞并支持攀爬
    const collisionResult = checkCollision(player.position, direction, true);
...
// 添加此函数来设置角色模型部件的透明度
function setCharacterPartsOpacity(isFirstPerson) {
    if (!character) return;
...
document.addEventListener('keydown', (event) => {
    ...
    if (event.ctrlKey) {
        if (event.key === '1') {
            ...
            const cameraPosition = camera.position.clone();
            blockReferences.forEach(block => {
                if (!block || block.isSky) return;
                const distanceToCamera = cameraPosition.distanceTo(block.position);
                block.visible = distanceToCamera <= renderSettings.currentRenderDistance &&
                    isInViewFrustum(block.position);
            });
            ...
            // 恢复角色部件可见性
            setCharacterPartsOpacity(false);
